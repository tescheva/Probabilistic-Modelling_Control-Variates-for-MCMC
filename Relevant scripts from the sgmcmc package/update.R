...
runSGMCMC = function( sgmcmc, paramsRaw, options ) {
    ...
    for ( i in 1:options$nIters ) {
        ...
        #times <<- c(times,(starttime - proc.time())[[3]]) #uncomment this line, once starting the grid search, and run this script again 
        #We added this line in order to be able to compare computational cost
        #The first time measurement appears after setup + first step
